...
static inline int phy_modify(struct phy_device *phydev, int devad, int regnum,
	u16 mask, u16 set)
{
	int ret = phy_read(phydev, devad, regnum);

	return (ret < 0) ? ret : phy_write(phydev, devad, regnum, (ret & ~mask) | set);
}
